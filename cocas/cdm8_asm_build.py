...
import re
import subprocess
from cdm8_internal import cocas
import argparse
import yaml
...
def main():
    ...
    link_result = subprocess.check_output(
        ['python', str(Path(os.path.dirname(os.path.abspath(__file__)) + '/cdm8_internal/cocol.py'))] + obj_filenames + ['-l']).decode('utf-8')
    print(link_result)
    sects = {}
    ...
    if not pathlib.Path(args.output_file.strip()).parent.exists():
        os.mkdir(pathlib.Path(args.output_file.strip()).parent.resolve())
