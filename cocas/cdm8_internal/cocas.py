...
def asmline(s, linum, passno):
    ...
    if cmd[0][0] != "id":
        ...
        if cat == un:
            ...
            if opcode in ("ldsa", "addsp", "setsp", "pushall", "popall") and args.v3: raise SE(cmd[next][2],
        ...
        if cat == mi:
            ...
            if opcode + "/" + str(parno) not in macros:
                raise SE(cmd[next][2],
                         "Number of params (" + str(parno) + ") does not match definition of macro " + opcode)
            ...
            newbody = []
            for s1 in mbody:
                if args != None and args.dbg: print(
                    "before => " + s1 + " ******* pars= " + str(pars) + "mvars=" + str(mvars))
...
def pretty_print(obj1, src, prtOP=True):
    ...
    if prtOP == True:
        print("\nCdM-8 Assembler v" + asmver + " <<<" + filename + '.asm>>> ' + time.strftime(
    ...
    for lnind in range(len(src)):
        ...
        if lnind + 1 <= len(src) - 1 and not ismex(s) and ismex(
                src[lnind + 1]):  # we are inside macro expansion and must not list it
        ...
        if obj == [] or obj[0][0] != lnind + 1:
            ...
            if secname == "":  # template
                if prtOP == True:
                    ...
                    retlist += (format(addr, "02x") + ': ' + " " * offset)[0:offset] + ' ' + format(ln1,
                                                                                                    '3d') + '  ' + s + '\n'
            while clist != []:
...
def commasep(tokens):
    ...
    while (k <= len(tokens) - 1):
        if tokens[k][0] == 'end':
            ...
            if tokens[k][0] == "id" and k <= len(tokens) - 3 and tokens[k + 1][0] == '.' and tokens[k + 2][
                0] == "id":  # template field
...
def ismstack(l, s):
    ...
    try:
        ...
        if tokens[0][0] == "id" and tokens[0][
            1] == "unique":  # not a macro stack operation but we keep it here for simplicity
            k = 1
            regfree = 4 * [True]
            regmvars = []
            howmany = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lx', dest='lstx', action='store_const', const=True, default=False,
